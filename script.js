...
function p4_initialize(inspiration) {
    ...
    if (inspiration.name == "yosemite"){
      initial = {type: "yosemite", opacity:{min: 0.1, max: 1}, intervals: 100};    
    }
  
    if (inspiration.name == "yellowstone"){
      initial = {type: "yellowstone", opacity:{min: 0.1, max: 1}, intervals: 100};    
    }
  
    if (inspiration.name == "niagarafalls"){
      initial = {type: "niagarafalls", opacity:{min: 0.1, max: 1}, intervals: 100};    
    }
  ...
  for (let i = 0; i < design.intervals; i++){
    ...
    for (let j = 0; j < design.intervals; j++){
      ...
      for (let n = 0; n < random(5,20); n++){
        ...
        fill(pxColor);
        square(random(xCoor, xCoor+xStep), random(yCoor, yCoor+yStep), 5);
      }
      yCoor += yStep;
    }
    xCoor += xStep;
